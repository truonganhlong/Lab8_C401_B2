...
def call_llm(prompt: str) -> str:
    """
    Gọi LLM để sinh câu trả lời (Option A — OpenAI).
    """
...
def rag_answer(
    ...
    abstain_keywords = ["do not know", "không biết", "không tìm thấy", "không có thông tin"]
    if any(k.lower() in answer.lower() for k in abstain_keywords):
        sources = []
    else:
        sources = list({
